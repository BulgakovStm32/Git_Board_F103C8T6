...
#define NRF24_GPIO_CE  	GPIOB
#define NRF24_PIN_CE	0

#define NRF24_GPIO_CSN 	GPIOB
#define NRF24_PIN_CSN	1
...
//*******************************************************************************************
//*******************************************************************************************
static void _spi_TransmitReceive(SPI_TypeDef *spi, uint8_t *pTxData, uint8_t *pRxData, uint16_t Size, uint32_t Timeout){

	//HAL_SPI_TransmitReceive(&hspi1, &addr, &dt, 1, 1000);
	//HAL_SPI_TransmitReceive(SPI_HandleTypeDef *hspi, uint8_t *pTxData, uint8_t *pRxData, uint16_t Size, uint32_t Timeout)
}
//**********************************************************
static void _spi_Transmit(SPI_TypeDef *spi, uint8_t *pData, uint16_t Size, uint32_t Timeout){

	//HAL_SPI_Transmit(SPI_HandleTypeDef *hspi, uint8_t *pData, uint16_t Size, uint32_t Timeout)
}
//**********************************************************
static void _spi_Receive(SPI_TypeDef *spi, uint8_t *pData, uint16_t Size, uint32_t Timeout){

	//HAL_SPI_Receive(SPI_HandleTypeDef *hspi, uint8_t *pData, uint16_t Size, uint32_t Timeout)
}
//**********************************************************
static void _gpio_WritePin(GPIO_TypeDef *gpio, uint16_t pin, uint16_t pinState){
...
{
	_gpio_WritePin(NRF24_GPIO_CSN, NRF24_PIN_CSN, level);
	delay_us(5);
}
//**********************************************************
void ce(uint8_t level)
{
	_gpio_WritePin(NRF24_GPIO_CE, NRF24_PIN_CE, level);
}
//**********************************************************
uint8_t read_register(uint8_t reg)
{
	...
	csn(LOW);
	_spi_TransmitReceive(NRF24_SPI, &addr, &dt, 1, 1000);
	_spi_TransmitReceive(NRF24_SPI, (uint8_t*)0xff, &dt, 1, 1000);
	csn(HIGH);
	return dt;
}
//**********************************************************
uint8_t write_registerMy(uint8_t reg, const uint8_t* buf, uint8_t len)
{
	...
	csn(LOW);
	_spi_TransmitReceive(NRF24_SPI, &addr, &status, 1, 100);
	_spi_Transmit(NRF24_SPI, (uint8_t*)buf, len, 100);
	csn(HIGH);
	return status;
}
//**********************************************************
uint8_t write_register(uint8_t reg, uint8_t value)
{
	...
	csn(LOW);
	_spi_TransmitReceive(NRF24_SPI, &addr, &status, 1, 100);
	_spi_Transmit(NRF24_SPI, &value, 1, 1000);
	csn(HIGH);
	return status;
}
//**********************************************************
uint8_t write_payload(const void* buf, uint8_t data_len, const uint8_t writeType)
{
	...
	csn(LOW);
	_spi_TransmitReceive(NRF24_SPI, &addr, &status, 1, 100);
	_spi_Transmit(NRF24_SPI, (uint8_t*)current, data_len, 100);

	while(blank_len--)
	{
		uint8_t empt = 0;
		_spi_Transmit(NRF24_SPI, &empt, 1, 100);
	}
...
uint8_t read_payload(void* buf, uint8_t data_len)
{
	...
	csn(LOW);
	_spi_Transmit(NRF24_SPI, &addr, 1, 100);
	_spi_Receive(NRF24_SPI, (uint8_t*)current, data_len, 100);

	while(blank_len--)
	{
		uint8_t empt = 0;
		_spi_Receive(NRF24_SPI, &empt, 1, 100);
	}
...
uint8_t spiTrans(uint8_t cmd)
{
	uint8_t status = 0;
	csn(LOW);
	_spi_TransmitReceive(NRF24_SPI, &cmd, &status, 1, 1000);
	csn(HIGH);
	return status;
}
//*******************************************************************************************
...
uint8_t getDynamicPayloadSize(void)
{
	...
	csn(LOW);
	addr = R_RX_PL_WID;
	_spi_TransmitReceive(NRF24_SPI, &addr, &result, 1, 1000);
	_spi_TransmitReceive(NRF24_SPI, (uint8_t*)0xff, &result, 1, 1000);
	csn(HIGH);
...
void toggle_features(void)
{
	...
	csn(LOW);
	_spi_Transmit(NRF24_SPI, &addr, 1, 1000);
	_spi_Transmit(NRF24_SPI, (uint8_t*)0x73, 1, 1000);
	csn(HIGH);
}
...
void writeAckPayload(uint8_t pipe, const void* buf, uint8_t len)
{
	...
	csn(LOW);
	_spi_Transmit(NRF24_SPI, &addr, 1, 1000);
	_spi_Transmit(NRF24_SPI, (uint8_t*)current, data_len, 1000);
	csn(HIGH);
}
