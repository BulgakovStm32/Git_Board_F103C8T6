...
//Указатели на контекст шины I2C. Нужны для работы обработчиков прерываний.
...
//*******************************************************************************************
//******************************СТАТИЧЕСКИЕ ФУНКЦИИ******************************************
//Ожидание установки флага в регистре SR1.
static I2C_State_t _i2c_WaitSetFlagSR1(I2C_TypeDef *i2c, uint32_t flag){

	volatile uint32_t count = 0;
	//---------------------
	while((i2c->SR1 & flag) == 0)//Ждем установки флага.
	{
		if(++count >= I2C_WAIT_TIMEOUT_SR1) return I2C_TIMEOUT;
	}
	return I2C_OK;
}
//**********************************************************
...
static I2C_State_t _i2c_ClearFlagCR1(I2C_TypeDef *i2c, __IO uint32_t flag){

	volatile uint32_t count = 0;
	//---------------------
	while((i2c->CR1 & flag) != 0)//Ждем сброса флага.
	{
		if(++count >= I2C_WAIT_TIMEOUT_CR1) return I2C_TIMEOUT;
	}
	return I2C_OK;
}
//**********************************************************
...
I2C_State_t I2C_StartAndSendDeviceAddr(I2C_TypeDef *i2c, uint8_t deviceAddr){
	...
	//сбрасываем бит ADDR (чтением SR1 и SR2):
	(void)i2c->SR1;
	(void)i2c->SR2;
	return I2C_OK;
}
...
I2C_State_t I2C_ReadData(I2C_TypeDef *i2c, uint8_t *pBuf, uint32_t len){
	...
	{
		//Вариант 1. По документу AN2824 STM32F10xxx I2C optimized examples
		//Этот вариант е читает даные из датчика AHT10.
//		uint32_t i;
//		for(i=0; i < (len-2); i++)
//		{
//			_i2c_WaitSetFlagSR1(i2c, I2C_SR1_RXNE);	//Wait for RXNE = 1
//			*(pBuf + i) = (uint8_t)i2c->DR;			//Read Data
//		}
...
//		i2c->CR1 |= I2C_CR1_ACK;				//ACK = 1 (to be ready for another reception)
		...
		//Вариант 2. По документу RM0008 Reference manual TM32F103xx
		//Figure 275. Method 2: transfer sequence diagram for master receiver when N>2 (стр.762)
		uint32_t i;
		for(i=0; i < (len-3); i++)
		{
			_i2c_WaitSetFlagSR1(i2c, I2C_SR1_RXNE);	//Wait for RXNE = 1
		...
		_i2c_WaitSetFlagSR1(i2c, I2C_SR1_BTF);	//Wait for BTF = 1
		i2c->CR1 &= ~I2C_CR1_ACK; 				//ACK = 0 - Фомирование NACK
		*(pBuf + i + 0) = (uint8_t)i2c->DR;		//Read DataN-2
		i2c->CR1 |= I2C_CR1_STOP; 				//STOP = 1 - Фомирование Stop
		*(pBuf + i + 1) = (uint8_t)i2c->DR;		//Read DataN-1
		_i2c_WaitSetFlagSR1(i2c, I2C_SR1_RXNE);	//Wait for RXNE = 1
		*(pBuf + i + 2) = (uint8_t)i2c->DR;		//Read DataN
		_i2c_ClearFlagCR1(i2c, I2C_CR1_STOP);	//Wait until STOP is cleared by hardware
		i2c->CR1 |= I2C_CR1_ACK;				//ACK = 1 (to be ready for another reception)
	}
	return I2C_OK;
...
void I2C_Master_Init(I2C_TypeDef *i2c, uint32_t remap, uint32_t speed){
	...
	i2c->CR1 |= I2C_CR1_ACK |	//Резрешаем ACK при приеме байта
				I2C_CR1_PE;		//Включение модуля I2C.
}
//**********************************************************
 ...
 */
I2C_State_t I2C_Master_SendData(I2C_TypeDef *i2c, uint8_t deviceAddr, uint8_t *pBuf, uint32_t len){
 ...
 *  Выход: статус передачи I2C_State_t.
 */
I2C_State_t I2C_Master_ReadData(I2C_TypeDef *i2c, uint8_t deviceAddr, uint8_t *pBuf, uint32_t len){
...
static void I2C_IT_Error(I2C_IT_t *i2cIt){
	...
	if(i2c->SR1 & I2C_SR1_AF)
	{
		...
		i2c->SR1 &= ~I2C_SR1_AF;			//Сброс флага ошибки.
