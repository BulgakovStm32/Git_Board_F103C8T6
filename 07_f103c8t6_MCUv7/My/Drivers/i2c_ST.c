...
void I2C_DMA_Write(I2C_TypeDef *i2c, uint8_t deviceAddr, uint8_t regAddr, uint8_t *pBuf, uint32_t size){
	...
	//------------------------------
	if(I2cDmaStateReg != I2C_DMA_READY) return;
...
//**********************************************************
uint32_t I2C_DMA_Read(I2C_TypeDef *i2c, uint8_t deviceAddr, uint8_t regAddr, uint8_t *pBuf, uint32_t size){
	...
	if(I2C_StartAndSendDeviceAddr(i2c, deviceAddr|I2C_MODE_READ) != 0)
	{
		i2c->CR1 |= I2C_CR1_STOP;//Формируем Stop
		dma->CCR &= ~DMA_CCR_EN; 	        //DMA Channel disable
		NVIC_DisableIRQ(DMA1_Channel7_IRQn);//Enable DMA1_Channel6_IRQn
