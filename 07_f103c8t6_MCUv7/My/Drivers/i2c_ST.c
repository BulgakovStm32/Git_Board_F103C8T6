...
void I2C_DMA_Init(I2C_TypeDef *i2c, uint32_t remap){
	...
	//DMA Init.
	//i2c->CR2 |= I2C_CR2_DMAEN; //DMAEN(DMA requests enable) — единица в этом бите разрешает делать запрос к DMA
							     //при установке флагов TxE или RxNE.
	//i2c->CR2 |= I2C_CR2_LAST;  // единица в этом бите разрешает DMA генерировать сигнал окончания передачи EOT(End of Transfer).
