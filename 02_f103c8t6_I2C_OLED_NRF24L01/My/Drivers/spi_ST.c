...
void SPI_Init(SPI_TypeDef *spi){
	...
	//--------------------
	//Включение тактирования и настройка портов
	if(spi == SPI1)
	{
		RCC->APB2ENR |= RCC_APB2ENR_IOPAEN |
				        RCC_APB2ENR_SPI1EN;  //Включение тактирования SPI1.


		GPIOA->CRL &= ~(GPIO_CRL_CNF5 | GPIO_CRL_CNF7);

		//PA5(SPI1_SCK) - выход, альтернативный режим push-pull.
		GPIOA->CRL |= GPIO_CRL_MODE5 | GPIO_CRL_CNF5_1;//тактирование 50МГц.
		...
		GPIOA->CRL |= GPIO_CRL_MODE7 | GPIO_CRL_CNF7_1;//тактирование 50МГц.
	}
	else if(spi == SPI2)
	{
		RCC->APB2ENR |= RCC_APB2ENR_IOPBEN;
		...
		//PB13(SPI2_SCK) - выход, альтернативный режим push-pull.
		GPIOB->CRH |= GPIO_CRH_MODE13 | GPIO_CRH_CNF13_1;//тактирование 50МГц.
		...
		//PB15(SPI2_MOSI) - выход, альтернативный режим push-pull.
		GPIOB->CRH |= GPIO_CRH_MODE15 | GPIO_CRH_CNF15_1;//тактирование 50МГц.
	}
	//--------------------
	//Настройка SPI.
